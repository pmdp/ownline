...
class PortForwardingAction(AbstractAction):
    ...
    def do_add(self, add_request):
        ...
        #todo: iptables binary path from config
        cmd = [self.iptables_binary, '-t', 'nat', '-I', 'VSERVER', '-s', ip_src + '/32', '-p', 'tcp', '-m', 'tcp',
        ...
        if ok:
            self.logger.info("Successful adding execution")
            ...
            raise Exception("Failed adding execution: stderr: {} stout: {}".format(err, out))
    ...
    def do_del(self, session):
        ...
        # todo: VSERVER or PREROUTING chains, get from config
        cmd = [self.iptables_binary, '-t', 'nat', '-D', 'VSERVER', '-s', ip_src + '/32', '-p', 'tcp', '-m', 'tcp',
               '--dport', str(port_dst), '-j', 'DNAT', '--to-destination', dst_lan]
    ...
    def do_flush(self):
        ...
        #todo: delete all rules from OWNLINE_PORT_FORWARDING chain
        pass
